...
before(async () => {
  ...
  await pool.query(`insert into auth.users (id,email) values ($1,'me@test'),($2,'other@test')
                    on conflict do nothing`, [USER, OTHER]);
});
...
beforeEach(async () => {
  ...
  await pool.query(`update user_settings set default_hourly_rate=100, max_timer_hours=8,
                    week_starts_on=1, next_invoice_number=1 where user_id=$1`, [USER]);
...
test('start → current → stop round trip', async () => {
  ...
  const { GET: current } = await import('../src/app/api/v1/timer/current/route.ts');
  const { POST: stop } = await import('../src/app/api/v1/timer/stop/route.ts');

  const started = await json(await start(req('/timer/start', { taskName: 'Write tests' })));
...
test('THE INVARIANT: a second start is rejected with 409 and returns the running entry', async () => {
  ...
  const first = await json(await start(req('/timer/start', { taskName: 'First' })));
  assert.equal(first.status, 201);

  const second = await json(await start(req('/timer/start', { taskName: 'Second' })));
  assert.equal(second.status, 409);
  assert.equal(second.body.code, 'TIMER_ALREADY_RUNNING');
  assert.equal(second.body.details.running.id, first.body.id,
...
test('a backdated stop before the start is rejected with a clear message', async () => {
  ...
  await start(req('/timer/start', { taskName: 'x', startedAt: '2026-09-11T12:00:00Z' }));
  const res = await json(await stop(req('/timer/stop', { endedAt: '2026-09-11T11:00:00Z' })));
...
test('runaway timer past the threshold is flagged but never auto-edited', async () => {
  ...
  const started = await json(await start(req('/timer/start', { taskName: 'Forgot', startedAt: long })));

  const now = await json(await current(req('/timer/current')));
  assert.equal(now.body.exceedsThreshold, true, 'surfaced to the client');
  assert.equal(now.body.entry.id, started.body.id);
  assert.equal(now.body.entry.startedAt, started.body.startedAt,
...
test('a running timer can be retitled and reassigned mid-run', async () => {
  const { POST: start } = await import('../src/app/api/v1/timer/start/route.ts');
  const { PATCH: patch } = await import('../src/app/api/v1/timer/current/route.ts');

  await start(req('/timer/start', { taskName: 'Old name' }));
  const res = await json(await patch(req('/timer/current', { taskName: 'New name' }, 'PATCH')));
...
test('a manual entry replayed with the same id is idempotent', async () => {
  ...
  const { rows } = await pool.query('select count(*)::int as n from time_entries');
  assert.equal(rows[0].n, 1, 'no duplicate was created');
});

test('a manual entry ending before it starts is rejected', async () => {
  const { POST: create } = await import('../src/app/api/v1/entries/route.ts');
  const res = await json(await create(req('/entries', {
    id: '018f0000-0000-7000-8000-000000000002',
    taskName: 'bad', startedAt: '2026-09-11T10:00:00Z', endedAt: '2026-09-11T09:00:00Z',
  })));
  assert.equal(res.status, 422);
});
...
test('editing an entry billed on a sent invoice returns 409 ENTRY_LOCKED', async () => {
  const { POST: create } = await import('../src/app/api/v1/entries/route.ts');
  const { PATCH: patch, DELETE: del } = await import('../src/app/api/v1/entries/[id]/route.ts');

  const id = '018f0000-0000-7000-8000-000000000003';
  await create(req('/entries', {
    id, taskName: 'Billed', startedAt: '2026-09-11T09:00:00Z', endedAt: '2026-09-11T10:00:00Z',
  }));
  ...
  const removed = await json(await del(req(`/entries/${id}`, undefined, 'DELETE'),
...
test('a missing entry is 404, not 500', async () => {
  ...
  const res = await json(await get(req(`/entries/${id}`), { params: Promise.resolve({ id }) }));
...
test('clients and projects create, list and archive', async () => {
  const { POST: createClient, GET: listClients } = await import('../src/app/api/v1/clients/route.ts');
  const { DELETE: archiveClient } = await import('../src/app/api/v1/clients/[id]/route.ts');
  const { POST: createProject, GET: listProjects } = await import('../src/app/api/v1/projects/route.ts');

  const c = await json(await createClient(req('/clients', { name: 'Northwind', hourlyRate: 150 })));
  assert.equal(c.status, 201);
  assert.equal(c.body.hourlyRate, 150, 'numeric comes back as a number, not a string');

  const p = await json(await createProject(req('/projects', {
    clientId: c.body.id, name: 'Lifecycle', hourlyRate: 175,
  })));
  ...
  const archived = await json(await archiveClient(req(`/clients/${c.body.id}`, undefined, 'DELETE'),
    { params: Promise.resolve({ id: c.body.id }) }));
  assert.equal(archived.status, 204);

  const active = await json(await listClients(req('/clients')));
  assert.equal(active.body.clients.length, 0, 'archived clients are hidden by default');

  const all = await json(await listClients(req('/clients?includeArchived=true')));
  assert.equal(all.body.clients.length, 1, 'but still retrievable — invoices reference them');
});

// ── settings ───────────────────────────────────────────────────────
test('settings update, and nextInvoiceNumber cannot be moved by a client', async () => {
  const { GET: get, PATCH: patch } = await import('../src/app/api/v1/settings/route.ts');

  const before = await json(await get(req('/settings')));
  assert.equal(before.status, 200);
  assert.equal(before.body.maxTimerHours, 8);

  const after = await json(await patch(req('/settings',
    { maxTimerHours: 10, defaultHourlyRate: 125, nextInvoiceNumber: 9999 }, 'PATCH')));
  assert.equal(after.status, 200);
  assert.equal(after.body.maxTimerHours, 10);
  assert.equal(after.body.defaultHourlyRate, 125);
  assert.equal(after.body.nextInvoiceNumber, 1,
...
test('calendar groups entries by LOCAL day', async () => {
  ...
  const utc = await json(await calendar(
    req('/calendar?from=2026-09-01T00:00:00Z&to=2026-09-30T00:00:00Z&tz=UTC')));
  assert.equal(utc.body.days[0].date, '2026-09-12');

  const sp = await json(await calendar(
    req('/calendar?from=2026-09-01T00:00:00Z&to=2026-09-30T00:00:00Z&tz=America/Sao_Paulo')));
  assert.equal(sp.body.days[0].date, '2026-09-11',
...
test('calendar rejects an inverted period', async () => {
  const { GET: calendar } = await import('../src/app/api/v1/calendar/route.ts');
  const res = await json(await calendar(
