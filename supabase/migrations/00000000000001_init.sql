...
create table time_entries (
  -- UUIDv7 generated by the CLIENT, so retries are idempotent and the
  -- offline outbox can replay safely.
  id            uuid primary key,
  user_id       uuid not null references auth.users(id) on delete cascade,
  ...
  client_updated_at timestamptz not null default now(),  -- outbox conflict resolution
  created_at    timestamptz not null default now(),
  updated_at    timestamptz not null default now(),
