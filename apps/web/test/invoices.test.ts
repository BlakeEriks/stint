...
async function seedEntry(opts: {
  ...
  const end = new Date(new Date(start).getTime() + (opts.hours ?? 1) * 3600_000);
  await pool.query(
    `insert into time_entries (id,user_id,project_id,task_name,started_at,ended_at,is_billable,rate_override)
     values ($1,$2,$3,$4,$5,$6,$7,$8)`,
    [opts.id, USER, PROJECT, opts.task ?? 'Work', start, end.toISOString(),
     opts.billable ?? true, opts.rateOverride ?? null],
  );
}
...
test('preview resolves rates and has no side effects', async () => {
  const { POST: preview } = await import('../src/app/api/v1/invoices/preview/route.ts');
  await seedEntry({ id: E(1), task: 'Design', hours: 2 });

  const res = await json(await preview(req('/invoices/preview', { clientId: CLIENT, ...PERIOD })));
  assert.equal(res.status, 200);
  assert.equal(res.body.lineItems.length, 1);
  assert.equal(res.body.lineItems[0].resolvedRate, 150, 'falls back to the client rate');
  ...
  const { rows: seq } = await pool.query(
    'select next_invoice_number n from user_settings where user_id=$1', [USER]);
...
test('preview reports unrated entries instead of billing them at zero', async () => {
  ...
  await pool.query('update user_settings set default_hourly_rate=null where user_id=$1', [USER]);
...
test('preview excludes non-billable and already-invoiced entries', async () => {
  ...
  await pool.query('update time_entries set invoice_id=$1 where id=$2',
    ['ff000000-0000-4000-8000-00000000000a', E(3)]);
...
test('preview rejects an inverted period', async () => {
  const { POST: preview } = await import('../src/app/api/v1/invoices/preview/route.ts');
  const res = await json(await preview(req('/invoices/preview',
    { clientId: CLIENT, periodStart: '2026-09-30', periodEnd: '2026-09-01' })));
...
test('generating allocates a number, freezes line items and locks entries', async () => {
  ...
  const res = await json(await create(req('/invoices', { clientId: CLIENT, ...PERIOD })));
  ...
  assert.equal(Number(items[0].resolved_rate), 150, 'the rate is frozen onto the line');

  const { rows: entries } = await pool.query(
    'select invoice_id from time_entries where id=$1', [E(1)]);
...
test('a generated invoice is immune to a later client rate change', async () => {
  ...
  const { rows } = await pool.query('select total from invoices where id=$1', [res.body.id]);
...
test('sequential generations produce gapless numbers', async () => {
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    const res = await json(await create(req('/invoices', {
      clientId: CLIENT, periodStart: `2026-09-0${i}`, periodEnd: `2026-09-0${i}`,
    })));
    assert.equal(res.status, 201);
...
test('entries billed on a DRAFT invoice stay editable', async () => {
  ...
  await pool.query('update time_entries set task_name=$1 where id=$2', ['Edited', E(1)]);
  const { rows } = await pool.query('select task_name from time_entries where id=$1', [E(1)]);
...
test('grouping mode is applied and frozen onto the invoice', async () => {
  ...
  await seedEntry({ id: E(2), task: 'Design', start: '2026-09-11T09:00:00Z', hours: 1 });
  await seedEntry({ id: E(3), task: 'Build', start: '2026-09-12T09:00:00Z', hours: 1 });

  const res = await json(await create(req('/invoices', {
    clientId: CLIENT, ...PERIOD, groupingMode: 'task',
  })));
...
test('status moves draft -> sent -> paid', async () => {
  ...
  const sent = await json(await setStatus(req('/s', { status: 'sent' }, 'PATCH'), ctx));
  assert.equal(sent.body.status, 'sent');
  assert.ok(sent.body.sentAt);

  const paid = await json(await setStatus(
    req('/s', { status: 'paid' }, 'PATCH'),
...
test('an invoice cannot return to draft once issued', async () => {
  ...
  await setStatus(req('/s', { status: 'sent' }, 'PATCH'),
    { params: Promise.resolve({ id: inv.body.id }) });
  const back = await json(await setStatus(req('/s', { status: 'draft' }, 'PATCH'),
    { params: Promise.resolve({ id: inv.body.id }) }));
...
test('voiding releases the entries so they can be re-billed', async () => {
  ...
  await setStatus(req('/s', { status: 'sent' }, 'PATCH'),
    { params: Promise.resolve({ id: inv.body.id }) });
  const voided = await json(await setStatus(req('/s', { status: 'void' }, 'PATCH'),
    { params: Promise.resolve({ id: inv.body.id }) }));
  assert.equal(voided.body.status, 'void');

  const { rows } = await pool.query('select invoice_id from time_entries where id=$1', [E(1)]);
  ...
  const { rows: inv2 } = await pool.query(
    'select invoice_number from invoices where id=$1', [inv.body.id]);
...
test('a draft can be deleted and releases its entries', async () => {
  const { POST: create } = await import('../src/app/api/v1/invoices/route.ts');
  const { DELETE: del } = await import('../src/app/api/v1/invoices/[id]/route.ts');
  await seedEntry({ id: E(1), hours: 1 });
  const inv = await json(await create(req('/invoices', { clientId: CLIENT, ...PERIOD })));

  const res = await json(await del(req('/i', undefined, 'DELETE'),
    { params: Promise.resolve({ id: inv.body.id }) }));
...
test('detail returns the frozen line items with the client', async () => {
  const { POST: create, GET: list } = await import('../src/app/api/v1/invoices/route.ts');
  const { GET: detail } = await import('../src/app/api/v1/invoices/[id]/route.ts');
  await seedEntry({ id: E(1), task: 'Design', hours: 2 });
  const inv = await json(await create(req('/invoices', { clientId: CLIENT, ...PERIOD })));

  const res = await json(await detail(req('/i'),
    { params: Promise.resolve({ id: inv.body.id }) }));
  assert.equal(res.status, 200);
...
test('the PDF renders from the frozen line items', async () => {
  ...
  assert.ok(bytes.length > 1000, `expected a real PDF, got ${bytes.length} bytes`);
