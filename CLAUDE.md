...
### Design docs are HTML, and they are authoritative

**Anything with a visual is specified in HTML, not Markdown** — written in the
app's own design system so the spec doubles as the visual reference.
`docs/design/brand.html` is identity; `docs/design/screens/*.html` is one doc
per route; `menubar.html` and `landing.html` are their own surfaces.
**Anything architectural or procedural stays Markdown** — `api.md`,
`data-model.md`, `architecture.md`, `deploying.md`, `local-dev.md`,
`tasks.md`, and `design/deriving-colour.md`, which is the OKLCH maths behind
the palette rather than a rule for using it. `principles.md` is product
decisions, so it stays Markdown too; nothing in it needs to be seen.

**A doc states the current final form.** Not options, not alternatives, not a
decision log. A doc may say *why* something is the way it is, because that
constrains the next change; it never says *what else was on the table* — no
"this used to say", no "was tried first", no refuting a proposal nobody made.
Unbuilt work goes to `tasks.md`. A rejection goes to `principles.md` **only if
someone would plausibly propose it again**; that file is a short list of live
refusals, not an archive of everything ever declined.

One screen, one doc — states live inside it as sections, so there is never a
`calendar-drag.html` disagreeing with `calendar.html`.

**`pnpm design` serves them** at `localhost:8778`, with an index and a nav
strip built by reading the directory on each request — a new doc appears as
soon as it is saved, and there is no list to keep in sync. A server rather
than `file://` because a doc links `_mockup.css` by relative path and renders
unstyled without one. The strip is injected at serve time, never written into
a file.
...
the gitignored `dist/`. It carries the palette, one class per type role, and
`.mark`. Never hand-edit it, and never hand-copy a palette into a doc's own
`:root` — that is the drift the token package exists to prevent, and it fails
silently, since a stale hex renders perfectly and merely misrepresents the app.
Start a new doc by copying `screens/_shell.html`.
...
web one and a Swift one that resemble each other — which is what they had
already become.
