...
- [ ] **`EntryDialog` adjusts its times by drag, not only by typing.** A strip
      between the project field and the time fields draws the entry on a
      cropped day: dragging an edge moves that end, dragging the body moves
      both, and the `start` and `end` inputs rewrite on every pointer move.
      Typing a time redraws the strip. One value, two controls — the calendar
      already offers the gesture and the dialog is where an entry is corrected
      deliberately, so it is the surface that most wants it.

      **The grid maths is already written and shared.** `instantAt()`,
      `movedTo()` and `resized()` in `@stint/core` carry `SNAP_MINUTES` 15 and
      `MIN_ENTRY_MINUTES` 15, so a strip built on them lands on the same
      quarter hour a calendar drag does. Use them rather than re-deriving from
      pixels; `resized()` is also what refuses to invert an entry, clamping to
      the floor instead.

      **The strip runs horizontally**, which is not the calendar's axis and is
      the one measurement worth stating: at the calendar's 44px/hour a day is
      ~700px of dialog to adjust two fields, and a vertical grid reads against
      the left-to-right Start → End pair directly beneath it.

      **The window is the entry ±2h, rounded out to a tick** — recomputed when
      the dialog opens and when a time is typed, never mid-drag, which would
      slide the ground under the pointer. Measured: a full day across a 460px
      strip draws a 90-minute entry 43px wide.

      **The handles straddle the block's edges rather than sitting inside
      them.** At `MIN_ENTRY_MINUTES` the block is ~27px however tightly the
      window crops, so two inset 10px handles leave 7px of body and the move
      gesture is unreachable on exactly the entry most likely to be a mistake
      worth dragging. Straddling gives each edge a 16px target and leaves the
      body whole at every length.

      **Nothing commits on release.** The strip writes the draft and `Save`
      writes the server, like every other field in this dialog — the
      calendar's `useEntryDrag` PATCHes on pointer-up only because it has no
      Save to wait for. It also needs no drag threshold: the block here is not
      also the control that opens the editor, so a press has nothing to
      disambiguate from.

      The block takes its client colour through `useProjectColors()`, never
      the accent — `Save` is this dialog's one accent. A billed entry gets the
      strip read-only, painted with no handles. The handles are `aria-hidden`
      and not focusable: `Start` and `End` with `step="900"` are already the
      keyboard path to the same two values, and a second tab stop for one
      value is two controls disagreeing about which is authoritative.

      Two things left open. **Overnight** is the dialog's existing rule —
      an end before the start rolls forward a day — and the strip cannot draw
      it; decide whether it renders to the window's edge and lets the fields
      hold the truth, or the strip hides itself. And whether a drag may
      **reach past the window** at all: stopping at the edge says hours-across
      -the-day is the field's job, which is the restrained reading.

      `docs/design/screens/entry-dialog.html` is the spec, and its strip is
      live — drag it before building it.
