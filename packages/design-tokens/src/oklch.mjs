...
export function gamutMap(L,C,h){
  ...
  let lo=0, hi=C;
  for(let i=0;i<40;i++){ const mid=(lo+hi)/2; if(inGamut(oklchToRgb(L,mid,h))) lo=mid; else hi=mid; }
  return lo;
}
export function hex(L,C,h){
  const c=gamutMap(L,C,h);
  const [r,g,b]=oklchToRgb(L,c,h).map(v=>Math.round(Math.max(0,Math.min(1,f(v)))*255));
  return '#'+[r,g,b].map(v=>v.toString(16).padStart(2,'0').toUpperCase()).join('');
}
export const relLum=(hx)=>{const[r,g,b]=[1,3,5].map(i=>fi(parseInt(hx.slice(i,i+2),16)/255));return 0.2126*r+0.7152*g+0.0722*b;};
export const contrast=(a,b)=>{const[x,y]=[relLum(a),relLum(b)].sort((m,n)=>n-m);return (x+0.05)/(y+0.05);};
